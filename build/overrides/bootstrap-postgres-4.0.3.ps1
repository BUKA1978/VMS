...
if (-not (Test-Path (Join-Path $DataDir 'PG_VERSION'))) {
    if (Test-Path $DataDir) {
        ...
        if ($existing.Count -gt 0) {
            $quarantine = "$DataDir.invalid-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
            Move-Item -Path $DataDir -Destination $quarantine -Force
            Write-Warning "Diretório PostgreSQL inválido movido para: $quarantine"
        }
...
# FVR VMS 4.0.3 - PostgreSQL dedicado
